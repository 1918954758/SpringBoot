...
- GitHub地址 [github-path](https://github.com/alibaba/druid/tree/master)
...
```
- 由于使用引入的druid-starter，之前自定义配置的druid需要注掉，然而只需要注释掉@Configuration、@Bean以及pom.xml中的druid数据源
...
```
- @ConditionalOnClass({DruidDataSource.class}) 
    - 容器中有DruidDataSource的时候，DruidDataSourceAutoConfigure里面的配置才生效
- @AutoConfigureBefore({DataSourceAutoConfiguration.class})
    - DataSourceAutoConfiguration配置之前，配置DruidDataSourceAutoConfigure
- @EnableConfigurationProperties({DruidStatProperties.class, DataSourceProperties.class})
    - Druid自动配置相关的配置类
- @Import({DruidSpringAopConfiguration.class, DruidStatViewServletConfiguration.class, DruidWebStatFilterConfiguration.class, DruidFilterConfiguration.class})
    - 导入其他的自动配置类
        ...
        - DruidStatViewServletConfiguration  监控页自动配置，默认开启
            - @ConditionalOnProperty(name = "spring.datasource.druid.stat-view-servlet.enabled", havingValue = "true")
        - DruidWebStatFilterConfiguration  web监控自动配置，默认开启
...
spring:
  datasource:
    #url: jdbc:mysql://localhost:3306/mysql?serverTimezone=Asia/Shanghai&characterEncoding=utf8
    #url: jdbc:mysql://localhost:3306/mysql?serverTimezone=GMT   该配置可以用于 SpringBoot 2.4.5
    url: jdbc:mysql://localhost:3306/testdb?serverTimezone=Asia/Shanghai&characterEncoding=utf8
    username: 'root'
    password: 'root'
    #type: com.zaxxer.hikari.HikariDataSource
    driver-class-name: com.mysql.cj.jdbc.Driver
    filters: wall,stat
    # config StatViewServlet
    druid:
      # 配置监控页功能
      stat-view-servlet:
        # stat-view-servlet-enabled default : false
        enabled: true
        login-username: druid
        login-password: druid
        # resetEnable default true
        reset-enable: false
      ...
      web-stat-filter:
        # web-stat-filter-enabled default : false
        enabled: true
        # 匹配哪些地址
        url-pattern: "/*"
        # 排除哪些
        exclusions: "*.js,*.gif,*.jpg,*.png,*.css,*.ico,/druid/*"
      # 底层Spring开启的功能  stat - sql监控功能，  wall - 防火墙监控功能
      filters: "wall,stat"
      filter:
        stat:
          # 配置慢查询sql日志（记录慢查询）
          log-slow-sql: true
          # 大于10毫秒的都属于慢查询（筛选哪些sql是慢查询）
          slow-sql-millis: 10
          enabled: true
        wall:
          enabled: true
      ...
      #SpringBean监控配置，说明请参考Druid Github Wiki，配置_Druid和Spring关联监控配置
      aop-patterns: com.zichen.admin.*
...
- 至此，引入druid-spring-boot-starter配置完成，
- 但是spring监控是空白的
- 解决spring监控是空白的问题：
...
![image-spring监控结果页](../image/spring监控结果页.png)
